"""Сессия и загрузка для отчёта «Коммерческие расходы» (3 файла, логика Тест Excel)."""
...
from almabi_commercial_expense_report import DETAIL_COLUMNS, build_commercial_expense_report
...
SESSION_ALMABI_COMMERCIAL_EXPENSE_REPORT = "almabi_commercial_expense_report"


def _meta_from_session(request: Request) -> dict[str, str] | None:
    stored = request.session.get(SESSION_ALMABI_COMMERCIAL_EXPENSE_REPORT)
...
def load_commercial_expense_report_payload(request: Request, settings: Settings) -> dict[str, object]:
    ...
    payload = build_commercial_expense_report(
        buh_path=buh_path,
        cost_path=cost_path,
...
async def store_commercial_expense_report_upload(
    request: Request,
    settings: Settings,
    *,
    buh_file: UploadFile,
    ...
    try:
        ...
        if not report["rows"]:
            ...
            raise HTTPException(
                status_code=400,
                detail="Не удалось собрать отчёт: в бухрегистре нет строк «Коммерческие расходы».",
            )
        ...
        request.session[SESSION_ALMABI_COMMERCIAL_EXPENSE_REPORT] = session_payload

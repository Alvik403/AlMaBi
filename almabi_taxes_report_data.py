"""Сессия и загрузка для отчёта «Налоги» (3 файла, логика Тест Excel)."""
...
from almabi_taxes_report import TAX_BUCKETS, TAX_DETAIL_COLUMNS, build_taxes_report
...
SESSION_ALMABI_TAXES_REPORT = "almabi_taxes_report"


def _meta_from_session(request: Request) -> dict[str, str] | None:
    stored = request.session.get(SESSION_ALMABI_TAXES_REPORT)
...
def _empty_payload() -> dict[str, object]:
    return {
        ...
        "summary": {
            ...
            "base_totals": {name: 0.0 for name in TAX_BUCKETS},
            "component_totals": {name: 0.0 for name in TAX_BUCKETS},
            "rates": {name: "0%" for name in TAX_BUCKETS},
            "month_count": 0,
            "amount_column": "Налог",
...
def load_taxes_report_payload(request: Request, settings: Settings) -> dict[str, object]:
...
async def store_taxes_report_upload(
    request: Request,
    settings: Settings,
    *,
    buh_file: UploadFile,
    ...
    try:
        ...
        if not report["rows"]:
            ...
            raise HTTPException(
                status_code=400,
                detail="Не удалось собрать отчёт: нет данных для расчёта налогов.",
            )
        ...
        request.session[SESSION_ALMABI_TAXES_REPORT] = session_payload

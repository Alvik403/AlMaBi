"""Сессия и загрузка для отчёта «Чистая прибыль» (3 файла, логика Тест Excel)."""
...
from almabi_net_profit_report import COMPONENT_ORDER, DETAIL_COLUMNS, build_net_profit_report
...
SESSION_ALMABI_NET_PROFIT_REPORT = "almabi_net_profit_report"


def _meta_from_session(request: Request) -> dict[str, str] | None:
    stored = request.session.get(SESSION_ALMABI_NET_PROFIT_REPORT)
...
def load_net_profit_report_payload(request: Request, settings: Settings) -> dict[str, object]:
    ...
    payload = build_net_profit_report(
        buh_path=buh_path,
        cost_path=cost_path,
...
async def store_net_profit_report_upload(
    request: Request,
    settings: Settings,
    *,
    buh_file: UploadFile,
    ...
    try:
        ...
        if revenue_file is not None and revenue_file.filename:
            revenue_original, revenue_stored, revenue_final = await _store_uploaded_file(
                upload=revenue_file,
                upload_dir=upload_dir,
                prefix="net-profit-report-revenue",
            )
            try:
        ...
        if not report["rows"]:
            ...
            raise HTTPException(
                status_code=400,
                detail="Не удалось собрать отчёт: нет данных для расчёта чистой прибыли.",
            )
        ...
        request.session[SESSION_ALMABI_NET_PROFIT_REPORT] = session_payload

"""Отчёт «Прочие доходы» — распределение по статье и документам (логика Тест Excel)."""
...
def _other_income_rows_from_buh(all_rows: list[dict[str, object]]) -> list[dict[str, object]]:
    rows: list[dict[str, object]] = []
    for row in all_rows:
        if normalize_text(row.get("Раздел")) != OTHER_INCOME_SECTION:
            continue
...
def build_other_income_hierarchy(rows: list[dict[str, object]]) -> list[dict[str, Any]]:
...
def build_other_income_report(
    *,
    ...
    rows = _other_income_rows_from_buh(all_rows)

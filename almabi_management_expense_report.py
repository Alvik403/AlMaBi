"""Отчёт «Управленческие расходы» — распределение по статье и документам (логика Тест Excel)."""
...
MANAGEMENT_EXPENSE_SECTION = "Управленческие расходы"
...
def _management_expense_rows_from_buh(all_rows: list[dict[str, object]]) -> list[dict[str, object]]:
    rows: list[dict[str, object]] = []
    for row in all_rows:
        if normalize_text(row.get("Раздел")) != MANAGEMENT_EXPENSE_SECTION:
            continue
        ...
        rows.append(
            {
                ...
                "Статья": _label(row.get("Статья дохода расхода"), default="Прочее"),
...
def build_management_expense_hierarchy(rows: list[dict[str, object]]) -> list[dict[str, Any]]:
...
def build_management_expense_report(
    *,
    buh_path: Path,
    ...
    rows = _management_expense_rows_from_buh(all_rows)
    total = sum(float(row.get("Сумма БУ") or 0) for row in rows)
    tax_totals = _tax_totals(rows)
    articles = {normalize_text(row.get("Статья")) for row in rows if row.get("Статья")}

"""Сессия и загрузка для отчёта «Операционная прибыль» (3 файла, логика Тест Excel)."""
...
from almabi_operating_profit_report import DETAIL_COLUMNS, build_operating_profit_report
...
SESSION_ALMABI_OPERATING_PROFIT_REPORT = "almabi_operating_profit_report"


def _meta_from_session(request: Request) -> dict[str, str] | None:
    stored = request.session.get(SESSION_ALMABI_OPERATING_PROFIT_REPORT)
...
def load_operating_profit_report_payload(request: Request, settings: Settings) -> dict[str, object]:
    ...
    payload = build_operating_profit_report(
        buh_path=buh_path,
        cost_path=cost_path,
...
async def store_operating_profit_report_upload(
    request: Request,
    settings: Settings,
    *,
    buh_file: UploadFile,
    ...
    try:
        ...
        if not report["rows"]:
            ...
            raise HTTPException(
                status_code=400,
                detail="Не удалось собрать отчёт: нет строк для расчёта операционной прибыли.",
            )
        ...
        request.session[SESSION_ALMABI_OPERATING_PROFIT_REPORT] = session_payload

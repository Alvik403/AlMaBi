"""Сессия и загрузка для отчёта «Прибыль/убыток до налогообложения» (3 файла, логика Тест Excel)."""
...
from almabi_profit_before_tax_report import COMPONENT_ORDER, DETAIL_COLUMNS, build_profit_before_tax_report
...
SESSION_ALMABI_PROFIT_BEFORE_TAX_REPORT = "almabi_profit_before_tax_report"


def _meta_from_session(request: Request) -> dict[str, str] | None:
    stored = request.session.get(SESSION_ALMABI_PROFIT_BEFORE_TAX_REPORT)
...
def load_profit_before_tax_report_payload(request: Request, settings: Settings) -> dict[str, object]:
    ...
    payload = build_profit_before_tax_report(
        buh_path=buh_path,
        cost_path=cost_path,
...
async def store_profit_before_tax_report_upload(
    request: Request,
    settings: Settings,
    *,
    buh_file: UploadFile,
    ...
    try:
        buh_original, buh_stored, buh_final = await _store_uploaded_file(
            ...
            prefix="profit-before-tax-report-buh",
        )
        try:
        ...
        if cost_file is not None and cost_file.filename:
            cost_original, cost_stored, cost_final = await _store_uploaded_file(
                upload=cost_file,
                upload_dir=upload_dir,
                prefix="profit-before-tax-report-cost",
            )
            try:
        ...
        if revenue_file is not None and revenue_file.filename:
            revenue_original, revenue_stored, revenue_final = await _store_uploaded_file(
                upload=revenue_file,
                upload_dir=upload_dir,
                prefix="profit-before-tax-report-revenue",
            )
            try:
        ...
        if not report["rows"]:
            ...
            raise HTTPException(
                status_code=400,
                detail="Не удалось собрать отчёт: нет строк для расчёта прибыли до налогообложения.",
            )
        ...
        request.session[SESSION_ALMABI_PROFIT_BEFORE_TAX_REPORT] = session_payload

"""Сессия и загрузка для отчёта «Прочие расходы» (3 файла, логика Тест Excel)."""
...
SESSION_ALMABI_OTHER_EXPENSE_REPORT = "almabi_other_expense_report"


def _meta_from_session(request: Request) -> dict[str, str] | None:
    stored = request.session.get(SESSION_ALMABI_OTHER_EXPENSE_REPORT)
...
def load_other_expense_report_payload(request: Request, settings: Settings) -> dict[str, object]:
    ...
    payload = build_other_expense_report(
        buh_path=buh_path,
        cost_path=cost_path,
...
async def store_other_expense_report_upload(
    request: Request,
    settings: Settings,
    *,
    buh_file: UploadFile,
    ...
    try:
        ...
        if not report["rows"]:
            ...
            raise HTTPException(
                status_code=400,
                detail="Не удалось собрать отчёт: в бухрегистре нет строк «Прочие расходы».",
            )
        ...
        request.session[SESSION_ALMABI_OTHER_EXPENSE_REPORT] = session_payload

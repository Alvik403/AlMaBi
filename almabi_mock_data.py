...
UNITS = [
    {"key": "units", "label": "ед.", "divisor": 1},
    {"key": "thousands", "label": "тыс.", "divisor": 1000},
    {"key": "millions", "label": "млн.", "divisor": 1000000},
]
...
def _month_values(values: list[int], plan_multiplier: float = 1.08) -> dict[str, dict[str, int]]:
    return {
        "Факт БУХ": dict(zip(MONTHS, values, strict=True)),
        "Факт НУ": dict(zip(MONTHS, [round(value * 0.96) for value in values], strict=True)),
...
def _leaf(name: str, level: int, month_values: list[int], *, sign: int = 1, id_prefix: str = "leaf") -> dict[str, Any]:
    return {
        "id": _next_id(id_prefix),
        "name": name,
        "level": level,
        "values": _month_values(_scale_months(month_values, 1, sign=sign)),
        "children": [],
    }
...
def _split_values(
    ...
    *,
    ...
    return [
        _leaf(name, level, _scale_months(month_values, weight), sign=sign, id_prefix=id_prefix)
        for name, weight in weights
    ]


def _build_org_tree(month_values: list[int], start_level: int, *, sign: int = 1, include_contract: bool) -> list[dict[str, Any]]:
                    ...
                    for contract, contract_weight in _ORG["contracts"][project]:
                        contract_children.append(
                            _leaf(
                                contract,
                                start_level + 4,
                                _scale_months(project_values, contract_weight),
                                sign=sign,
                                id_prefix="contract",
                            )
                        )
                project_children.append(
                    {
                        ...
                        "name": project,
                        "level": start_level + 3,
                        "values": _month_values(_scale_months(group_values, project_weight, sign=sign)),
            ...
            group_children.append(
                {
                    ...
                    "name": group,
                    "level": start_level + 2,
                    "values": _month_values(_scale_months(direction_values, group_weight, sign=sign)),
        ...
        nodes.append(
            {
                ...
                "name": direction,
                "level": start_level + 1,
                "values": _month_values(_scale_months(month_values, direction_weight, sign=sign)),
...
def _build_cost_branch(month_values: list[int], *, sign: int = -1) -> list[dict[str, Any]]:
    ...
    for cost_name, weight, drill_mode in _COST_TYPES:
        ...
        if drill_mode == "org_short":
            children = _build_org_tree(branch_values, start_level=3, sign=sign, include_contract=False)
        else:
            children = [
                {
...
def _build_benefit_branch(month_values: list[int], *, sign: int = 1, with_articles: bool = False) -> list[dict[str, Any]]:
    ...
    for benefit_name, weight in _BENEFIT_SPLIT:
        ...
        if with_articles:
            children = _split_values(benefit_values, _OTHER_ARTICLES, level=3, sign=sign, id_prefix="article")
...
def build_summary_rows() -> list[dict[str, Any]]:
    ...
    return [
        _kpi_node("Выручка", revenue, children=_build_org_tree(revenue, start_level=2, include_contract=True)),
        _kpi_node("Себестоимость", cost, sign=-1, children=_build_cost_branch(cost, sign=-1)),
        _kpi_node("Коммерческие расходы", commercial, sign=-1, children=_build_benefit_branch(commercial, sign=-1)),
        _kpi_node("Управленческие расходы", admin, sign=-1, children=_build_benefit_branch(admin, sign=-1)),
        _kpi_node("Операционная прибыль", operating, children=_build_benefit_branch(operating)),
        _kpi_node("Прочие доходы", other_income, children=_build_benefit_branch(other_income, with_articles=True)),
        _kpi_node("Прочие расходы", other_expense, sign=-1, children=_build_benefit_branch(other_expense, sign=-1, with_articles=True)),
        _kpi_node("Прибыль/убыток до налогообложения", pbt, sign=-1, children=_build_benefit_branch(pbt, sign=-1)),
        _kpi_node("Налоги", taxes, sign=-1, children=_build_benefit_branch(taxes, sign=-1)),
        _kpi_node("Чистая прибыль", net, sign=-1, children=_build_benefit_branch(net, sign=-1)),
    ]
...
COST_BY_MONTH = _chart_series([52_400_000, 18_300_000, 61_200_000, 74_800_000])

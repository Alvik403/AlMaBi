"""Сессия и загрузка для отчёта «Себестоимость» (3 файла, логика Тест Excel)."""
...
from almabi_cost_report import DETAIL_COLUMNS, build_cost_report
...
SESSION_ALMABI_COST_REPORT = "almabi_cost_report"


def _meta_from_session(request: Request) -> dict[str, str] | None:
    stored = request.session.get(SESSION_ALMABI_COST_REPORT)
...
def load_cost_report_payload(request: Request, settings: Settings) -> dict[str, object]:
...
async def store_cost_report_upload(
    request: Request,
    settings: Settings,
    *,
    buh_file: UploadFile,
    ...
    try:
        ...
        if not report["rows"]:
            ...
            raise HTTPException(
                status_code=400,
                detail="Не удалось собрать отчёт: в бухрегистре нет строк «Себестоимость».",
            )
        ...
        request.session[SESSION_ALMABI_COST_REPORT] = session_payload

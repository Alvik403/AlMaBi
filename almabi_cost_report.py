"""Отчёт «Себестоимость» — распределение по направлению, группе, проекту и документам (логика Тест Excel)."""
...
def _cost_rows_from_buh(all_rows: list[dict[str, object]]) -> list[dict[str, object]]:
    rows: list[dict[str, object]] = []
    for row in all_rows:
        if normalize_text(row.get("Раздел")) != COST_SECTION:
            continue
...
def build_cost_hierarchy(rows: list[dict[str, object]]) -> list[dict[str, Any]]:
...
def build_cost_report(
    *,
    ...
    rows = _cost_rows_from_buh(all_rows)

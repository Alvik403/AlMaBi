"""Отчёт «Коммерческие расходы» — распределение по статье и документам (логика Тест Excel)."""
...
COMMERCIAL_EXPENSE_SECTION = "Расходы на продажу"
...
def _commercial_expense_rows_from_buh(all_rows: list[dict[str, object]]) -> list[dict[str, object]]:
    rows: list[dict[str, object]] = []
    for row in all_rows:
        if normalize_text(row.get("Раздел")) != COMMERCIAL_EXPENSE_SECTION:
            continue
...
def build_commercial_expense_hierarchy(rows: list[dict[str, object]]) -> list[dict[str, Any]]:
...
def build_commercial_expense_report(
    *,
    buh_path: Path,
    ...
    rows = _commercial_expense_rows_from_buh(all_rows)

"""Отчёт «Прочие расходы» — распределение по статье и документам (логика Тест Excel)."""
...
def _other_expense_rows_from_buh(all_rows: list[dict[str, object]]) -> list[dict[str, object]]:
    rows: list[dict[str, object]] = []
    for row in all_rows:
        if normalize_text(row.get("Раздел")) != OTHER_EXPENSE_SECTION:
            continue
...
def build_other_expense_hierarchy(rows: list[dict[str, object]]) -> list[dict[str, Any]]:
...
def build_other_expense_report(
    *,
    buh_path: Path,
    ...
    rows = _other_expense_rows_from_buh(all_rows)

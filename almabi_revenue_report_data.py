"""Сессия и загрузка для отчёта «Выручка» (3 файла, логика Тест Excel)."""
...
from almabi_revenue_report import DETAIL_COLUMNS, build_revenue_report
from almabi_test_excel_data import (
    SESSION_ALMABI_TEST_EXCEL_BUH,
    SESSION_ALMABI_TEST_EXCEL_COST,
    SESSION_ALMABI_TEST_EXCEL_REVENUE,
...
SESSION_ALMABI_REVENUE_REPORT = "almabi_revenue_report"


def _meta_from_session(request: Request) -> dict[str, str] | None:
    stored = request.session.get(SESSION_ALMABI_REVENUE_REPORT)
...
def load_revenue_report_payload(request: Request, settings: Settings) -> dict[str, object]:
...
async def store_revenue_report_upload(
    request: Request,
    settings: Settings,
    *,
    buh_file: UploadFile,
    ...
    try:
        ...
        if not report["rows"]:
            ...
            raise HTTPException(
                status_code=400,
                detail="Не удалось собрать отчёт: в бухрегистре нет строк «Выручка».",
            )
        ...
        request.session[SESSION_ALMABI_REVENUE_REPORT] = session_payload
